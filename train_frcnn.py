...
from keras import backend as K           #backend后端
from keras.optimizers import Adam, SGD, RMSprop       #优化函数
...
def write_log(callback, names, logs, batch_no):
    for name, value in zip(names, logs):        #zip将对象中对应的元素打包成元组，然后返回元组组成的列表。
        summary = tf.Summary()
        summary_value = summary.value.add()        #添加数据到tensorboard中
        summary_value.simple_value = value          #此两行定义add中的参数值
...
def my_write_log(callback, names, logs, batch_no):
    for name, value in zip(names, logs):
        writer = tf.summary.create_file_writer(callback.log_dir)
        with writer.as_default():
            for step in range(batch_no):
                tf.summary.scalar(name, value, step=step)     #写一个标量summary
                # callback.writer.flush()


sys.setrecursionlimit(40000)#手动调整递归调用深度

parser = OptionParser()      #构造optionparser的对象

parser.add_option("-p", "--path", dest="train_path", help="Path to training data.")  #往对象中加option
...
for epoch_num in range(num_epochs):
    ...
    while True:
        ...
        print(Y.__len__(), 'ddddddddddddddddddddddddddddd')
        loss_rpn = model_rpn.train_on_batch(X, Y)
        # write_log(callback, ['rpn_cls_loss', 'rpn_reg_loss'], loss_rpn, train_step)
        my_write_log(callback, ['rpn_cls_loss', 'rpn_reg_loss'], loss_rpn, train_step)
        ...
        my_write_log(callback, ['detection_cls_loss', 'detection_reg_loss', 'detection_acc'], loss_class, train_step)
        train_step += 1
